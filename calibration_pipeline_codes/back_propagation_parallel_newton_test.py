...
class multi_ligand_cob(single_ligand_model):    
    ...
    def construct_thetas_full(self, log_theta):  
        theta_unscaled = torch.exp(log_theta+self.scale)
...
class ObjectiveWithHistory:
    def __init__(self, model):
       # function that returns gradient
    ...
    def fun_and_grad(self, x):
        if self._last_x is None or not (x == self._last_x).all():
            # Compute and cache
            self._last_x = x.copy()
            with torch.no_grad():
                self.model.scaled_log_theta.copy_(torch.from_numpy(x).to(self.model.scaled_log_theta.device))
            self.model.zero_grad()
            loss = self.model()
            self._last_fun = loss.item()
            loss.backward()
            self._last_jac = self.model.scaled_log_theta.grad.detach().cpu().numpy()
        return self._last_fun, self._last_jac


    def callback(self, xk, res=None):
        # Called after each iteration
        self.loss_history.append(self._last_fun)
        #print(f"Loss at Epoch {len(self.loss_history)} is {self._last_fun}")
...
def train_single_seed(seed, optimize_scheme, theta, neighbours_list,\
                      ...
                      min_vals, max_vals,num_epochs, regularization_param):
...
# ---- Training Loop ----
    model = multi_ligand_cob(scaled_var, scale,\
                         neighbours_list, cell_densities, mesh_prop, I_exp,\
                         seg_shared, seg_1, seg_2, seg_3, min_vals, max_vals, regularization_param)
    obj = ObjectiveWithHistory(model)
   ...
   # initial_params = torch.exp(scale*{name: param.detach().cpu().clone() for name, param in model.named_parameters()}['scaled_log_theta'])
    ...
    x0=scaled_var.detach().cpu().numpy() #initial_guess
    result = minimize(fun=obj.fun_and_grad,
                  x0=x0,
                  jac=True,
                  method='trust-constr',
                  hess=BFGS(),
    ...
    training_history = obj.loss_history
    final_parameters = np.exp(result.x+scale.detach().cpu().numpy())
    ...
    return final_loss, training_history, final_parameters, total_time
...
if __name__ == "__main__":
    ...
    relative_learning_rates[MC_fixed_param_indices]= 1#10
    relative_learning_rates[SMC_param_fixed_indices]=1# 5
    ...
    #basic_learning_rate = [5e-3,1e-2,2e-2,3e-2]
    epochs = int(1e3)
    regularization_param = 0.002#[0.001,0.005, 0.01, 0.015, 0.02]
    local_histories = np.zeros([local_seedings.shape[0], epochs])
    local_final_loss = np.zeros([local_seedings.shape[0]])
    local_parameters = np.zeros([local_seedings.shape[0], d])
    local_running_times = np.zeros([local_seedings.shape[0]])
    ...
    for i in range(local_seedings.shape[0]):
        ...
        final_loss, loss_history, final_parameters, running_time =\
            train_single_seed(seed_num, "Newton", theta,\
        ...
        local_histories[i,:len(loss_history)] = loss_history
        local_final_loss[i] = final_loss
        local_parameters[i,:] = final_parameters
        local_running_times[i] = running_time
    ...
    if rank==0:
        counter=0
        final_histories = np.zeros([parameter_array.shape[0], epochs ])
        final_loss = np.zeros([parameter_array.shape[0]])
        final_parameter_values = np.zeros([parameter_array.shape[0], d])
        final_running_times = np.zeros([parameter_array.shape[0]])
        ...
        with open(f"final_ML_results_{parameter_array.shape[0]}_seeds_{epochs}_epochs_ADAM.pkl", "wb") as f:
            pkl.dump({"loss": final_loss, "parameters": final_parameter_values,\
                         "histories": final_histories, "running_times": final_running_times,\
                             "regularization_params": regularization_param}, f)
