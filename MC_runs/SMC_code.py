...
# ----------------------
def evaluate_realization(r_i, I_emp, neighbours, mesh_prop, cell_dens, epsilon):
...
epsilon_0 = 0.4
T = int(30)
N = int(3e3) #number of particles
population_scores = np.zeros([T,N])
epsilon_vector = np.zeros(T)
epsilon_vector[0] = epsilon_0
#different processing for T=0
theta_t_minus = np.zeros([N,d_var])
acceptance_threshold = 1e-20
cov_scaling_constant=1.2
particle_realizations = np.zeros((T,N,d_var))
max_attempts = int(1500)
adaptive_percentile = 60



def initial_theta0_sample(i, std_prior, mean_prior, epsilon, fixed_indices, fixed_MLE, prior_dis):
    ...
    while (score is None) and attempts<=max_attempts:
        theta_star = prior_dis.sample(1)[0]#sample from prior
        unnormalized_sample[variable_indices] = theta_star*std_prior+mean_prior
...
def iterative_theta_sample(i, weights_old, theta_t_minus, kernel_cov_matrix,
                           std_prior, mean_prior, epsilon, fixed_indices, fixed_MLE, prior_dis):
    unnormalized_sample = np.zeros(d_total)
    unnormalized_sample[fixed_indices] = fixed_MLE
    variable_indices = np.setdiff1d(np.arange(d_total), fixed_indices)
    attempts=0
    ...
    while (score is None) and attempts<=max_attempts:
        ...
        unnormalized_sample[variable_indices] = std_prior*theta_star+mean_prior
        prior_pdf_i = np.exp(gmm_prior.score_samples(theta_star.reshape(1, -1)))[0]
    ...
    if score is None:
        raise RuntimeError(f"Failed to get valid score after {max_attempts} attempts.")
...
if __name__ == "__main__":
    ...
    results = Parallel(n_jobs=num_workers)(delayed(initial_theta0_sample)(i, 
    ...
    print("starting SMC for T>0")
    for t in range(1,T):
        epsilon_t = np.percentile(population_scores[t-1,:], adaptive_percentile)
        epsilon_vector[t] = epsilon_t  
        results = Parallel(n_jobs=num_workers)(delayed(iterative_theta_sample)(i, \
                  weights_old, theta_t_minus, kernel_cov_matrix, \
                  std_prior, mean_prior, epsilon_t, fixed_param_indices,\
                  fixed_param_MLE, gmm_prior) for i in range(N))
        theta_t_minus = np.array([r[0] for r in results])
        population_scores[t, :] = np.array([r[1] for r in results])
        weights_new = np.array([r[2] for r in results])
        ...
        if weight_normalization == 0 or not np.isfinite(weight_normalization):
            raise ValueError("Sum of new weights is zero or non-finite; cannot normalize.")
        weights_old = np.exp(np.log(np.clip(weights_new, 1e-300, None))-np.log(weight_normalization))      
        kernel_cov_matrix = np.cov(theta_t_minus,rowvar=False)*cov_scaling_constant +\
             1e-10 * np.eye(d_var)
        
        print(f"Checkpointing after population {t + start_index}")
        save_checkpoint(particle_realizations, population_scores, t + start_index, epsilon_vector)

    if not np.all(particle_realizations==0):
